...
class IMUtils {
    ...
    try {
      switch (message.contentType) {
        ...
        case MessageType.merger:
          final title = message.mergeElem?.title;
          content = isNotNullEmptyStr(title)
              ? '[${StrRes.mergeForward}] $title'
              : '[${StrRes.mergeForward}]';
          break;
        case MessageType.custom:
          ...
          switch (customType) {
            ...
            case CustomMessageType.call:
              final parsed = parseCustomMessage(message);
              if (parsed is Map && parsed['content'] != null) {
  ...
  static Future<String?> resolveVoicePlayPath({
    String? soundPath,
    String? sourceUrl,
    String? cacheKey,
  }) async {
    if (isNotNullEmptyStr(soundPath)) {
      var path = soundPath!.trim();
      if (path.startsWith('http://') || path.startsWith('https://')) {
        sourceUrl ??= path;
      } else {
        path = await toFilePath(path);
    ...
    if (!isNotNullEmptyStr(sourceUrl)) {
      return null;
    }

    final url = HttpUtil.normalizeObjectUrl(sourceUrl!.trim());
    final uri = Uri.tryParse(url);
    final urlPath = uri?.path ?? url;
    final dotIndex = urlPath.lastIndexOf('.');
    final ext = dotIndex != -1 && urlPath.length - dotIndex <= 5
        ? urlPath.substring(dotIndex)
        : '.m4a';
    final name = '${cacheKey ?? url.hashCode}$ext';
    final dir = await createTempDir(dir: 'voice');
    final cachePath = '$dir/$name';
    ...
    try {
      ...
      Logger.print('resolveVoicePlayPath download failed: $e $s');
      rethrow;
    }
    return null;
  }
