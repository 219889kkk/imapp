...
class ChatReadTagView extends StatelessWidget {
  ...
  int get _needReadMemberCount {
    final groupMemberCount =
        message.attachedInfoElem?.groupHasReadInfo?.groupMemberCount ?? 0;
    if (groupMemberCount > 0) return groupMemberCount;
    final hasReadCount =
  ...
  Widget build(BuildContext context) {
    if (message.isSingleChat) {
      return (isRead ? StrRes.hasRead : StrRes.unread).toText
        ..style =
            (isRead ? Styles.ts_8E9AB0_12sp : Styles.ts_bubbleUnread_12sp);
    } else {
      if (_needReadMemberCount == 0) return const SizedBox();
      if (_unreadCount <= 0) return const SizedBox();
      return GestureDetector(
        onTap: onTap,
        behavior: HitTestBehavior.translucent,
        child: sprintf(StrRes.nPersonUnRead, [_unreadCount]).toText
          ..style = Styles.ts_bubbleUnread_12sp,
      );
    }
  }
}
