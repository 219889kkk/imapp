    ...
    try {
      ...
      switch (message.contentType) {
        ...
        case MessageType.revokeMessageNotification:
          final revokedMap = map['messageRevoked'] is Map<String, dynamic>
              ? map['messageRevoked'] as Map<String, dynamic>
              : map;
          final ntf = RevokedInfo.fromJson(revokedMap);
          final revokerName =
              ntf.revokerNickname ?? ntf.sourceMessageSenderNickname ?? '';
          if (revokerName.isEmpty) {
            return StrRes.revokeMsg.toText..style = Styles.ts_8E9AB0_12sp;
          }
          return RichText(
            textAlign: TextAlign.center,
            text: TextSpan(
              text: revokerName,
              style: Styles.ts_0089FF_12sp,
              recognizer: ntf.revokerID == null
                  ? null
                  : (TapGestureRecognizer()
                    ..onTap = () => onTapUserProfile((
                          userID: ntf.revokerID!,
                          name: revokerName,
                          faceURL: null,
                          groupID: groupID,
                        ))),
