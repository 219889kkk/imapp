...
class GroupSetupLogic extends GetxController {
  ...
  Future<void> _refreshConversationInfo() async {
    if (Get.arguments['conversationInfo'] != null) return;
    ...
    groupInfo.update((val) {
      if (temp.groupID != null) {
        val?.groupID = temp.groupID!;
      }
      val?.groupName = temp.showName;
      val?.faceURL = temp.faceURL;
    });
  }
  ...
  Future<void> editAnnouncement() async {
    final inputCtrl = TextEditingController(text: groupInfo.value.notification);
    final result = await Get.dialog<String>(
      AlertDialog(
        title: Text(StrRes.groupAc),
        content: TextField(
    ...
    await LoadingView.singleton.wrap(
      asyncFunction: () => _modifyGroupInfo(notification: result),
    );
    groupInfo.update((val) {
